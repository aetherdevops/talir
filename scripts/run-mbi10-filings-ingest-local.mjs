#!/usr/bin/env node
/**
 * One-shot: apply migrations → ingest MBI10 audited + quarterly filings (OCR) → regenerate derived JSON
 * Requires .env.local:
...
run('Ingest audited financials (MBI10 + OCR)', 'npm', ['run', 'ingest:fundamentals'], {
    TALIR_DOCUMENT_STORE: 'supabase',
    TALIR_SCOPE: 'MBI10',
    TALIR_OCR_FUNDAMENTALS: '1',
})

run('Ingest quarterly / H1 results (MBI10 + OCR)', 'npm', ['run', 'ingest:results'], {
    TALIR_DOCUMENT_STORE: 'supabase',
    TALIR_SCOPE: 'MBI10',
    TALIR_OCR_RESULTS: '1',
})

run('Generate fundamentals (merge from Supabase)', 'npm', ['run', 'generate:fundamentals'], {
    TALIR_DOCUMENT_STORE: 'supabase',
})

run('Generate results calendar', 'npm', ['run', 'generate:results-calendar'])

console.log('\nDone. Check lib/data/derived_fundamentals.json and Supabase seinet_documents.')
